...
class NameField(CharField):
    ...
    def formfield(self, **kwargs):
        defaults = {
            ...
            'regex': re.compile("^[A-Za-z]{1,250}$"),
            'max_length': self.max_length,
            'error_messages': {
                'invalid': _(u'Enter a valid name'),  # NOQA
            }
        }
...
class InitialsField(CharField):
    ...
    def formfield(self, **kwargs):
        defaults = {
            ...
            'max_length': self.max_length,
            'error_messages': {
                'invalid': _(u'Enter valid initials. Must be 2-3 letters, all '
...
class OmangField(CharField):
    ...
    """See EncryptedIdentityField!
        field for omang. If getting an ID that may alos be something other
        than an Omang,  use IdentityField along with IdentityTypeField
    """
    ...
    def formfield(self, **kwargs):
        defaults = {
            ...
            'max_length': self.max_length,
            'error_messages': {
                'invalid': _(u'Enter a valid Omang. Must be 9 numbers. Note '
                             'that digit 5 represents gender.'),
            }
        }
        defaults.update(kwargs)
...
class IdentityTypeField(CharField):
    ...
    """
        ...
        identity = models.CharField(
            ...
            max_length=25,
            unique=True,
            help_text=_("Use Omang, Passport number, driver's license number 
            or Omang receipt number")
            )
        ...
        Use the value of identity_type to check the cleaned value of 
        identity at the form level.
...
class CellPhoneField(CharField):
    ...
    """
    description = _("Custom field for Cellphone number")

    def __init__(self, *args, **kwargs):
        kwargs.setdefault('verbose_name', _('Mobile'))
        kwargs.setdefault('editable', True)
        kwargs.setdefault('max_length', 8)
        kwargs.setdefault('help_text', _('The format is 72000000'))
        CharField.__init__(self, *args, **kwargs)
    ...
    def formfield(self, **kwargs):
        defaults = {
            ...
            'error_messages': {
                'invalid': _(u'Enter a valid cellphone number. Allowed '
                             'prefixes are 71-78.'),
            }
        }
        defaults.update(kwargs)
