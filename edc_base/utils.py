...
def get_safe_random_string(length=12, safe=None, allowed_chars=None):
    ...
    allowed_chars = (allowed_chars or
                     ...
                     'EFGHIJKLMNOPQRTUVWXYZ012346789!@#%^&*()?<>.,[]{}')
    if safe:
...
def get_utcnow_time():
    return arrow.utcnow().time


def to_arrow_utc(dt, timezone=None):
    """Returns a datetime after converting date or datetime from the given
       timezone string to \'UTC\'."""
    try:
        dt.date()
    ...
    if born_utc.datetime > reference_dt_utc.datetime:
        raise AgeValueError(
            'Reference date {} {} precedes DOB {} {}. Got {}'.format(
                reference_dt, str(reference_dt.tzinfo), born, timezone,
                rdelta))
    return rdelta
...
def formatted_age(born, reference_dt=None, timezone=None):
    if born:
        ...
        elif age_delta.years == 0 and age_delta.months > 0 and\
                age_delta.months <= 2:
